...
export default class HyperLinkTool implements InlineTool {
  ...
  private showLinkDialog(selectedText: string, range: Range): void {
    ...
    backdrop.classList.add('hyperlink-backdrop');
    bodyFragment.appendChild(backdrop);

    const modal = document.createElement('div');
    modal.classList.add('hyperlink-modal');
    bodyFragment.appendChild(modal);

    modal.innerHTML = `
      <div class="hyperlink-modal-content">
        <h3 class="hyperlink-modal-title">Insert Link</h3>
        <div class="hyperlink-form-group">
          <label class="hyperlink-label">Link Text:</label>
          <input 
            type="text" 
            id="hyperlink-text" 
            class="hyperlink-input" 
            value="${this.escapeHtml(selectedText)}" 
          />
        </div>
        <div class="hyperlink-form-group">
        ...
        <div class="hyperlink-modal-actions">
          <button id="hyperlink-cancel" class="hyperlink-btn hyperlink-btn-cancel">
            Cancel
          </button>
          <button id="hyperlink-insert" class="hyperlink-btn hyperlink-btn-insert">
            Insert
          </button>
        </div>
      </div>
    ...
    const cancelBtn =
      modal.querySelector<HTMLButtonElement>('#hyperlink-cancel');
    const insertBtn =
      modal.querySelector<HTMLButtonElement>('#hyperlink-insert');
    const urlInput = modal.querySelector<HTMLInputElement>('#hyperlink-url');
    const textInput = modal.querySelector<HTMLInputElement>('#hyperlink-text');
    ...
    insertBtn?.addEventListener('click', () => {
      ...
      if (text && url) {
        this.insertLink(text, url, range);
      }
