...
// Handles Backspace and Delete operations for custom block & text selections
...
// Main hook for cross-block selection and marquee interactions
export function useCrossBlockSelection(
  ...
  useEffect(() => {
    ...
    // Resolve mouse coordinates to a target text node and block
    ...
    // Construct a DOM Range between two selection points
    ...
    // Highlight selected text with custom selection rects
    ...
    // Mouse down handler: start text selection or marquee box
    ...
    const onMouseUp = (e: MouseEvent) => {
      ...
      if (!isDrag && mouseDownTarget) {
        ...
        if (tableWrap || isBelowRedactor) {
          ...
          if (targetBlockIdx !== -1) appendOrFocusNextBlock(targetBlockIdx);
        }
      }

      state.selecting = state.isMarquee = false;
    };

    // Render the marquee selection box
    ...
    // Copy handler for marquee block selection or text range selection
    ...
    // Keyboard handler for table navigation and block deletion
