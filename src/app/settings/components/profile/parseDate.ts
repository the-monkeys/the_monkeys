...
export function formatDate(dateString:string) {
  ...
  return formattedDate.replace(/\d{1,2}:\d{2}:\d{2} (AM|PM)/, '00:00:00 GMT+0530 (India Standard Time)');
}
...
export function convertDate(dateString:string) {
  ...
  // Check if the date is valid
  if (isNaN(date.getTime())) {
      throw new Error('Invalid date format. Please use YYYY-MM-DD or YYYY-MM-DD HH:MM:SS +0000 +0000.');
  }
